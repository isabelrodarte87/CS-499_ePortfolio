 ...
 * Enhancements includes improved code structure, data validation, and better adherence to Java conventions.
 * 
...
public class UserInputWindow implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == button) {
            ...
            if(userText.getText().trim().isEmpty() || userText2.getText().trim().isEmpty() 
                    || userText3.getText().trim().isEmpty() || userText4.getText().trim().isEmpty()){
                JOptionPane.showMessageDialog(null, "Please fill all the fields with valid data");
                return;
            }
            // Get user input from text fields, and convert text into the appropriate types
            double initialInvestment = Double.parseDouble(userText.getText());
            double monthlyDeposit = Double.parseDouble(userText2.getText());
            double interestRate = Double.parseDouble(userText3.getText());
            int numberOfYears = Integer.parseInt(userText4.getText());

            // Create a new window to display the results
            FinancialDetailsWindow resultWindow = new FinancialDetailsWindow();

            // Call the calculateBalanceWithoutMonthlyDeposit method from the calculations class
            double withoutDepositResult = FinancialCalculator.calculateBalanceWithoutMonthlyDeposit(initialInvestment, interestRate, numberOfYears, resultWindow);

            // Call the balanceWithMonthlyDeposit method from the calculations class
            double withDepositResult = FinancialCalculator.calculateBalanceWithMonthlyDeposit(initialInvestment, monthlyDeposit, interestRate, numberOfYears, resultWindow);
